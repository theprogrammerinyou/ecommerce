...
const Summary = ({ payment }) => {
  const {
    BasketState: { mrp, discount, total, itemsCount },
  } = useBasket();
  ...
  return (
    <>
      ...
      <Button size="md" isWide={true} onClick={handleNavigate}>
        {stage[nextCheckoutState]?.key}
      </Button>
      <div className={styles.save_offer}>
        <p>
          You will save <span>Rs.250</span> from the regular market price
        </p>
      </div>
    </>
  );
