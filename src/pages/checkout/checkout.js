...
const Basket = () => {
    const { BasketState: { items: BasketItems, mrp, discount, total, address }, removeAllFromBasket } = useBasket()
        ...
        <section className={styles.container}>
            {BasketItems.length === 0 && pathname !== "/checkout/order-placed"
                ...
                : <div className={styles.wrapper}>
                    {pathname !== "/checkout/order-placed" && <div className={styles.progress_container}>
                        <span></span>
                        <div className={`${styles.icons} ${pathname === "/checkout/basket" && styles.active}`}>
                            <RiShoppingBasket2Fill />
                        </div>
                        <div className={`${styles.icons} ${pathname === "/checkout/address" && styles.active}`}>
                            <BiMap />
                        </div>
                        <div className={`${styles.icons} ${pathname === "/checkout/payment" && styles.active}`}>
                            ₹
                        </div>
                    </div>}
                    <Outlet />
                    {pathname !== "/checkout/order-placed" && <aside className={styles.summary_container}>
